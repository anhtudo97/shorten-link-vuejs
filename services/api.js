...
export const shortendLink = (destination) => {
...
export const checkSlashTag = (slash, domainId) => {
  return api.get(`/links/exists?slashtag=${slash}&domainId=${domainId}`);
};
...
export const getDomainsWorkspace = (
  token,
  workspaceId,
  page,
  status = false,
  ...
  return api.get(
    `/workspaces/${workspaceId}/domains?page=${page}&perPage=${perpage}&withDefault=${status}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
