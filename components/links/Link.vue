...
<v-row class="link-detail px-3">
                ...
                <div class="d-flex justify-space-between align-center shortened-info">
                    ...
                    <div>
                        ...
                        <div class="d-flex link-services">
                            <v-tooltip top nudge-left="10">
                                <template v-slot:activator="{ on, attrs }">
                                    <a :href="`https://${shorten}`" target="_blank">
                                        <img :src="require('@/assets/icons/route-solid.svg')" alt="route" class="ml-5" v-bind="attrs" v-on="on" />
                                    </a>
                                </template>
                            ...
                            <v-tooltip top nudge-left="10">
                                <template v-slot:activator="{ on, attrs }">
                                    <button type="button" v-bind="attrs" v-on="on" @click="doCopy">
                                        <img :src="require('@/assets/icons/clone-solid.svg')" alt="route" class="ml-5" v-bind="attrs" v-on="on" />
                                    </button>
                                </template>
                            ...
                            <v-tooltip top nudge-left="10">
                                <template v-slot:activator="{ on, attrs }">
                                    <div @click.stop="openModalUpdate">
                                        <img :src="require('@/assets/icons/edit-solid.svg')" alt="route" class="ml-5" v-bind="attrs" v-on="on" />
                                    </div>
                                </template>
                                <span>Edit</span>
                            </v-tooltip>
                            <v-tooltip top nudge-left="10">
                                <template v-slot:activator="{ on, attrs }">
                                    <div @click.stop="isRemoveModal = true">
                                        <img :src="require('@/assets/icons/trash-solid.svg')" alt="route" class="ml-5" v-bind="attrs" v-on="on" />
                                    </div>
                                </template>
                                <span>Remove</span>
                            </v-tooltip>
    ...
    <v-dialog v-model="models.isOpen" class="dialog" max-width="900" :fullscreen="width < 600 ? true : false">
        <DetailLinkModal :id="id" :slashtag="slashtag" @refetchLinks="$emit('refetchLinks')" @closeModalDetailLink="closeModalDetailLink" />
    </v-dialog>
    <v-dialog v-model="modalEditLink" class="link__dialog" max-width="900" :fullscreen="width < 600 ? true : false">
        <CreateNewLink :id="id" :edit="true" @refetchLinks="$emit('refetchLinks')" @closeModalAddNewLink="closeModalEditNewLink" />
    </v-dialog>
    ...
    <v-snackbar key="2" v-model="showAlertCopy" centered color="success" timeout="2000">
        Link copied
        <template v-slot:action="{ attrs }">
            <v-btn color="white" text v-bind="attrs" aria-label="close" @click="showAlert = false">Close</v-btn>
        </template>
...
export default {
    ...
    methods: {
        ...
        doCopy() {
            this.$copyText(`http://${this.shorten}`).then((this.showAlertCopy = true));
        },
        async removeLink() {
            try {
                this.loading = true;
